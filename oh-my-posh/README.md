...
- zsh  (this repo's `zshrc`): `eval "$(oh-my-posh init zsh --config "$DOTFILES/oh-my-posh/theme.jsonc")"`
- bash (this repo's `bashrc`): `eval "$(oh-my-posh init bash --config "$DOTFILES/oh-my-posh/theme.jsonc")"`
...
```sh
oh-my-posh print primary --config "$DOTFILES/oh-my-posh/theme.jsonc"
```

Render with realistic state (exit code, pwd):

```sh
oh-my-posh print primary --config "$DOTFILES/oh-my-posh/theme.jsonc" --status 1 --pwd ~/projects/Dotfiles
```
